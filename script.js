...
function showErrorAlert(message) {
  ...
  alertDiv.className =
    "fixed top-4 right-4 bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded shadow-lg";
  alertDiv.innerHTML = `
...
function updateDashboardStats() {
  ...
  const totalProducts = storeDetails.reduce(
    (sum, store) => sum + store.products.length,
    0
  );
  const totalRevenue = storeDetails.reduce(
    (sum, store) =>
      sum +
      store.products.reduce(
        (pSum, product) => pSum + product.price * product.quantity,
        0
      ),
    0
  );
...
function generateStoreCards() {
  ...
  storeDetails.forEach((store) => {
    ...
    card.className =
      "p-6 bg-gray-50 rounded-xl hover:shadow-md transition-all cursor-pointer";
    card.innerHTML = `
            ...
            <p class="text-gray-600 text-sm mb-4">Floor: ${
              store.floor_no
            } | Reg: #${store.registration_no}</p>
            <div class="text-gray-600 text-sm mb-4">Created on: ${new Date(
              store.creation_date
            ).toLocaleDateString()}</div>
            <div class="flex justify-between items-center text-sm">
            <span class="text-gray-600">Products: ${totalProducts}</span>
            <button class="text-indigo-600 hover:text-indigo-700" onclick="showStoreDetails('${
              store.id
            }')">
                View Details
            </button>
...
function showStoreDetails(storeId) {
  ...
  modalContent.className =
    "bg-white rounded-xl p-6 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto";
                            ...
                            ${store.products
                              .map(
                                ...
                                <tr class="border-t border-gray-200">
                                    ...
                                    <td class="px-4 py-2">${formatCurrency(
                                      product.price
                                    )}</td>
                                    <td class="px-4 py-2">${formatCurrency(
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const addStoreBtn = document.querySelector("button i.fas.fa-plus");
  if (addStoreBtn) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!dashboardLink || !storesLink || !dashboardView || !storesView || !storesGrid) {
    ...
    function switchView(showDashboard) {
        ...
        storesView.classList.toggle('hidden', showDashboard);
        
        // Update active state of navigation links
        dashboardLink.classList.toggle('bg-gray-100', showDashboard);
        dashboardLink.classList.toggle('text-gray-700', showDashboard);
        storesLink.classList.toggle('bg-gray-100', !showDashboard);
        storesLink.classList.toggle('text-gray-700', !showDashboard);
    }
    ...
    storesLink.addEventListener('click', (e) => {
        e.preventDefault();
        switchView(false);
        // Move the store cards to the stores grid
        const storeButtons = document.getElementById('store-buttons');
        if (storeButtons && storesGrid) {
            storesGrid.innerHTML = storeButtons.innerHTML;
        }
    });
});
